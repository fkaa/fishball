...
#define VIRTUAL_CHUNK_SIZE (1 << CHUNK_SIZE_EXPONENT)
#define VIRTUAL_CHUNK_MASK (CHUNK_SIZE - 1)
...
struct FbVoxelChunk {
    struct FbVoxel *slices[VIRTUAL_CHUNK_COUNT];
};

struct FbVoxelMemory {
    struct FbVoxel **chunk_pages;
};
...
void VXL_new_world(struct FbVoxelWorldConfig cfg, struct FbVoxelWorld **world)
{
    ...
    for (int i = 0; i < VIRTUAL_CHUNK_COUNT; i++) {
        ARRAY_push(w.memory.chunk_pages, malloc(VIRTUAL_CHUNK_VOLUME));
    }
    for (int i = 0; i < VIRTUAL_CHUNK_COUNT; i++) {
        for (int j = 0; j < VIRTUAL_CHUNK_VOLUME; j++) {
            w.memory.chunk_pages[i][j] = (struct FbVoxel) { .type = (i * j) % 255 };
        }
    }
...
unsigned short VXL_morton_LUT[48] = {
...
{
    unsigned int virtual_chunk_index = (x + 16 * (y + 16 * z)) >> VIRTUAL_CHUNK_SIZE_EXPONENT; 
    unsigned int index = VXL_encode_voxel_position(x, y, z);

    struct FbVoxel *virtual_chunk = chunk->slices[virtual_chunk_index];
    return virtual_chunk[index];
}
...
struct FbVoxel VXL_find_voxel(struct FbVoxelWorld *world, int x, int y, int z)
{
    ...
    if (chunk) {
        voxel = VXL_chunk_get(chunk, x & CHUNK_MASK, y & CHUNK_MASK, z & CHUNK_MASK);
    }
